...
const ChooseFromMenu: React.FC = () => {
  return (
    <section className="bg-black min-h-screen">
      <div className="container mx-auto px-4 py-16">
        {/* Header Section */}
        <div className="text-center mb-12">
          <p className="text-[#FF9F0D] text-3xl md:text-4xl mb-2 animate-fade-in">
            Choose & pick
          </p>
        ...
        <nav className="mb-16">
          <ul className="flex flex-wrap justify-center gap-4 md:gap-8 text-white">
            {MenuCategories.map((category, index) => (
              <li
                key={category}
                className={`cursor-pointer hover:text-[#FF9F0D] transition-colors ${index === 0 ? 'text-[#FF9F0D] font-bold' : ''}`}
              >
                {category}
              </li>
            ))}
