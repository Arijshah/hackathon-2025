...
export default function Navbar() {
  ...
  // Separate tabs for food and shop categories
  const [activeTab, setActiveTab] = useState('food'); // Default to 'food'
  ...
  return (
    <header className="z-50 w-full bg-[#0D0D0D] sticky top-0 px-4 sm:px-6 lg:px-[15.62%] py-4 lg:py-7">
      <nav className="flex items-center justify-between relative">
        ...
        {/* Tabs for Food and Shop */}
        <div className="hidden lg:flex items-center gap-6 text-white">
          <button
            className={`px-4 py-2 rounded-md ${
              activeTab === 'food' ? 'bg-[#FF9F0D] text-black' : 'hover:text-[#FF9F0D]'
            }`}
          ...
          <button
            className={`px-4 py-2 rounded-md ${
              activeTab === 'shop' ? 'bg-[#FF9F0D] text-black' : 'hover:text-[#FF9F0D]'
            }`}
